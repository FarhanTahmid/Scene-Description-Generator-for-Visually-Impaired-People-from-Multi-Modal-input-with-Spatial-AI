...
def get_image_dataloader(mode='train',
                         ...
                         images_path=os.environ[
                             'PROJECT_DIRECTORY'] + 'Dataset/coco/images/',
...
class ImageDataset(Dataset):
    ...
    def __init__(self,
                 mode='train',
                 coco_set=2014,
                 images_path=os.environ['PROJECT_DIRECTORY'] + 'Dataset/coco/images/',
                 vocab_path='Data/processed/coco_vocab.pkl',
                 captions_path='Data/processed/coco_captions.csv',
                 batch_size=32,
                 max_len=35,
        ...
        if (not load_captions or
            ...
            self.df = self.df[self.df['embedded_caption'].apply(
                lambda x: x[max_len]) == self.vocab(self.vocab.pad_word)]
            self.df['embedded_caption'] = self.df[
        ...
        if self.load_features:
            self.df['filename'] = self.df['filename'].apply(
                lambda x: x.replace('.jpg', '_' + model + '.pkl'))
        ...
        if self.preload and self.load_features:
            self.im_embeddings = torch.empty(
                len(self.files), 1, embedding_size)
            ...
            for i, im_id in enumerate(self.files):
                ...
                cap_embeddings = self.df[self.df['id'] == im_id][
