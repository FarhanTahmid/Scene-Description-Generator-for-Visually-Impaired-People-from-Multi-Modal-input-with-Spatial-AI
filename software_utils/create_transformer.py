...
class Resize(object):
    ...
    def __repr__(self):
        """Get parameters of Resize class."""
...
class Pad(object):
    ...
    def __init__(self, padding, fill=0, padding_mode='constant'):
        ...
        if isinstance(padding, Sequence) and len(padding) not in [2, 4]:
            raise ValueError('Padding must be an int or a 2,' +
                             ' or 4 element tuple, not a ' +
...
def create_transformer(augment=False):
    ...
    return transforms.Compose([
        ...
        # Get 224 x 224 crop from random location
